...
class ASTRONODE:
    def __init__(self, module_tx, module_rx, module_serial_port_name=None):
        global is_cpython, is_circuitpython, is_micropython
        self._serialPort = None
        self._debug_on = False

        if module_tx is not None and module_rx is not None:
            if is_micropython:
                ...
                if module_tx == 0 and module_rx ==1:
                    self._serialPort = UART(board.GP0, board.GP1, baudrate=9600, bits=8, parity=None, stop=1)
                elif module_tx == 5 and module_rx ==6:
                    self._serialPort = UART(board.GP5, board.GP6, baudrate=9600, bits=8, parity=None, stop=1)
        elif module_serial_port_name is not None and is_cpython:
    ...
    def _text_to_hex(self, text):
        ...
        if isinstance(text, str):
            text = text.encode()
        try:
            resp = binascii.hexlify(text).decode('ascii')
        except:
            pass
        return resp
    ...
    def _generate_message(self, payload, include_message_id=False, id=None):
        ...
        if include_message_id:
            # modify by LamVT
            id = id if id is not None else ('{:0>4s}'.format((hex(random.randint(0, 65535)))[2:6]))
            m = id
